...
ObjectTracker::ObjectTracker(){
	...
	trackingDistanceThreshold = 50.0f;
}
...
int ObjectTracker::trackObjects(const vector<Region> &srcRegionArray, unsigned int detectedRegionCount){
	...
	for ( unsigned int j = 0; j < detectedRegionCount; j++ ){
		...
		for ( unsigned int i = 0; i < trackingObjectArray.size(); i++ ){
			...
			if ( trackingObjectArray[i].id ){
				distance = sqrtf( 
					( trackingObjectArray[i].x - srcRegionArray[j].centroid.x ) * ( trackingObjectArray[i].x - srcRegionArray[j].centroid.x ) 
				...
				if ( distance < trackingDistanceThreshold ){
					...
					trackingObjectArray[i].boundingBox = srcRegionArray[j].boundingRect;
					trackingObjectArray[i].size = (float)srcRegionArray[j].size;
					trackingObjectArray[i].angle = srcRegionArray[j].angle;
					trackingObjectArray[i].shape = srcRegionArray[j].shape;
					trackingObjectArray[i].lastUpdatedTime = timer->systemTime;
					flagUpdated = 1;
					objectCounter++;
					break;
	...
	for ( unsigned int i = 0; i < trackingObjectArray.size(); i++ ){
		if ( trackingObjectArray[i].id != 0 ){
			if ( trackingObjectArray[i].lastUpdatedTime + 20 < timer->systemTime ){
